...
{
    ui->setupUi(this);
    connect(ui->label_img, &my_label::draw_rectangle, this, &MainWindow::show_rectangle);
}
...
void MainWindow::on_btn_choose_clicked()
{
    ...
    if(QString::compare(file_name, QString()) != 0)
    {
        ...
        if(valid)
        {
            original = ui->label_img->image.scaled(ui->label_img->size());
            ui->label_img->setPixmap(QPixmap::fromImage(original));
        }
    } else
    {
